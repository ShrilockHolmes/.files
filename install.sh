...
if [[ $i3 -eq 'y'||'yes' ]]; then
  if pacman -V >> /dev/null; then
    sudo pacman -S rofi i3 i3blocks ttf-roboto ttf-roboto-mono-nerd ttf-jetbrains-mono-nerd picom dunst rofi-calc rofi-emoji xss-lock bat trash-cli z stow scrot alacritty
    if yay; then
